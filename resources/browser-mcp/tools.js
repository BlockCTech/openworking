...
// MCP-local tool names stay unprefixed because OpenCode v2 prefixes them with the server name when
// exposing functions to the model (`click` -> `browser_click`). The resulting public surface must
// stay in sync with the browser-use skill's permissions; each local tool maps 1:1 to a host op.
const TOOLS = [
  {
    name: "navigate",
    op: "navigate",
