...
// The UI may edit any saved project's memory file, but the running runtime only needs a reload
// when a save affects global memory or the actually active project's memory.
function shouldReloadRuntimeForMemorySave(scope, projectId, activeProjectId, runtimeStatus) {
  return runtimeStatus === "running" && (scope === "global" || !!(projectId && projectId === activeProjectId))
}
