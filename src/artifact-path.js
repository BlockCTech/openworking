...
function translatedArtifactRequestInfo(artifactPath) {
  const requestedArtifact = path.resolve(String(artifactPath))
  const parsed = path.parse(requestedArtifact)
  const extension = parsed.ext.toLowerCase()
  ...
  return {
    ...
    relativePath: path.basename(requestedArtifact),
    name: path.basename(requestedArtifact),
    extension,
...
function previewTranslationArtifact(projectPath, artifactPath, maxBytes = 2 * 1024 * 1024) {
  if (!fs.existsSync(path.resolve(String(artifactPath)))) {
    return { ...translatedArtifactRequestInfo(artifactPath), previewMode: "missing", truncated: false }
  }
