...
function downloadInstaller({ downloadUrl, destDir, onProgress, maxRedirects = 5 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const defaultExtension = process.platform === "win32" ? "exe" : "dmg"
    const fileName = path.basename(parsed.pathname) || `OpenWorking-update.${defaultExtension}`
...
// Verifies a downloaded NSIS installer before it is executed. The expected
// publisher is embedded into signed Windows builds by scripts/windows-release.js,
// so a compromised download endpoint cannot substitute another validly-signed
// executable. `exec` and `platform` are injectable for unit tests on non-Windows.
function verifyWindowsInstaller({
  ...
  if (platform !== "win32") throw new Error("Windows installer verification is only supported on Windows")
  if (!installerPath || path.extname(installerPath).toLowerCase() !== ".exe") {
    throw new Error("Expected a Windows .exe installer")
  }
  if (!publisherName) throw new Error("Expected Windows publisher is not configured")

  const script = [
    "$sig = Get-AuthenticodeSignature -LiteralPath $env:OPENWORKING_INSTALLER_TO_VERIFY",
    "$publisher = if ($sig.SignerCertificate) { $sig.SignerCertificate.GetNameInfo([System.Security.Cryptography.X509Certificates.X509NameType]::SimpleName, $false) } else { '' }",
    "@{ Status = $sig.Status.ToString(); Publisher = $publisher; Subject = if ($sig.SignerCertificate) { $sig.SignerCertificate.Subject } else { '' } } | ConvertTo-Json -Compress"
  ].join("; ")
  const raw = exec(
    "powershell.exe",
    ["-NoProfile", "-NonInteractive", "-Command", script],
    {
      ...
      env: {
        ...process.env,
        OPENWORKING_INSTALLER_TO_VERIFY: path.resolve(installerPath)
      }
    }
  )
  ...
  try {
    ...
    throw new Error("Could not read the Windows installer signature")
  }
  if (result.Status !== "Valid") {
    throw new Error(`Windows installer signature is not valid (${result.Status || "unknown"})`)
  }
  if (result.Publisher !== publisherName) {
    throw new Error(`Windows installer publisher mismatch: expected "${publisherName}", got "${result.Publisher || "unknown"}"`)
  }
  return result
}

// Starts the verified NSIS installer silently and independently of the running
// Electron process. The caller quits the app immediately afterwards so NSIS can
// replace locked files and then launch the new version.
function launchWindowsInstaller({
  installerPath,
  spawnProcess = spawn,
  platform = process.platform
} = {}) {
  if (platform !== "win32") throw new Error("Windows installer launch is only supported on Windows")
  const child = spawnProcess(installerPath, ["/S", "--updated"], {
    detached: true,
    stdio: "ignore",
    windowsHide: false
  })
