...
// npm is npm.cmd on Windows, and Node refuses to spawn a .cmd directly since the CVE-2024-27980
// fix (EINVAL) — which would make installing this connector impossible there. Go through
// `cmd.exe /c` rather than shell:true, so pkgSpec and the install dir survive as single arguments
// even when they contain spaces (shell:true concatenates argv unescaped; see Node DEP0190).
function npmInvocation(args) {
  if (process.platform !== "win32") return ["npm", args]
  return [process.env.COMSPEC || "cmd.exe", ["/c", "npm.cmd", ...args]]
}

function runNpmInstall(installDir, pkgSpec, pathValue) {
  return new Promise((resolve, reject) => {
    execFile(
      ...npmInvocation(["install", pkgSpec, "--no-audit", "--no-fund", "--save-exact"]),
      {
        cwd: installDir,
