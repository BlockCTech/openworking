...
test("Windows x64 and arm64 retain the existing dual-URL contract", async () => {
  await withServer(
    jsonHandler(200, {
      update_available: true,
      update_required: false,
      latest_version: "2.0.0",
      download_url: "https://example.com/OpenWorking-2.0.0-arm64.exe",
      download_url_intel: "https://example.com/OpenWorking-2.0.0-x64.exe"
    }),
    async (baseURL) => {
      const x64 = await checkDesktopVersion({ currentVersion: "1.0.0", platform: "win32", arch: "x64", baseURL })
      const arm64 = await checkDesktopVersion({ currentVersion: "1.0.0", platform: "win32", arch: "arm64", baseURL })
      assert.match(x64.downloadUrl, /-x64\.exe$/)
      assert.match(arm64.downloadUrl, /-arm64\.exe$/)
    }
  )
})

test("Windows installer verification requires a valid signature from the configured publisher", () => {
  const calls = []
  const exec = (command, args, options) => {
    calls.push({ command, args, options })
    return JSON.stringify({ Status: "Valid", Publisher: "OpenWorking", Subject: "CN=OpenWorking" })
  }
  const result = verifyWindowsInstaller({
    installerPath: "C:\\Temp\\OpenWorking.exe",
    publisherName: "OpenWorking",
    exec,
    platform: "win32"
  })
  assert.equal(result.Publisher, "OpenWorking")
  assert.equal(calls[0].command, "powershell.exe")
  assert.match(calls[0].options.env.OPENWORKING_INSTALLER_TO_VERIFY, /OpenWorking\.exe$/)
...
test("Windows installer verification rejects invalid and non-exe payloads", () => {
  const invalidExec = () => JSON.stringify({ Status: "HashMismatch", Publisher: "OpenWorking" })
  assert.throws(
    () => verifyWindowsInstaller({
      installerPath: "C:\\Temp\\OpenWorking.exe",
      publisherName: "OpenWorking",
  ...
  assert.throws(
    () => verifyWindowsInstaller({
      installerPath: "C:\\Temp\\OpenWorking.zip",
      publisherName: "OpenWorking",
      platform: "win32"
    }),
    /Expected a Windows \.exe/
  )
})

test("Windows installer launch is detached and silent", () => {
  const calls = []
