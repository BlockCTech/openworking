...
// The browser bridge targets macOS only: it writes into ~/Library/Application Support, shells out
// through a #!/bin/sh launcher and relies on POSIX permission bits. The tests that assert those
// literal paths and modes are skipped elsewhere rather than weakened to fit another platform.
const darwinOnly = { skip: process.platform !== "darwin" && "macOS-only browser bridge" }

test("chromeNativeMessagingDir resolves the macOS Chrome stable path", darwinOnly, () => {
...
test("installHost writes a tokened sidecar, an executable launcher and a pinned manifest", darwinOnly, () => {
  const home = tempHome()
...
test("installHost tightens a pre-existing token file with looser permissions to 0600", darwinOnly, () => {
  const home = tempHome()
