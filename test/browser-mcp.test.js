...
test("MCP-local tool names produce the browser-use skill's public surface after server prefixing", () => {
  ...
  assert.deepEqual(names.map((name) => `browser_${name}`), [
    "browser_click",
    "browser_navigate",
    "browser_read",
    "browser_screenshot",
...
test("opForTool maps MCP-local tool names to host ops and rejects unknown tools", () => {
  assert.equal(opForTool("navigate"), "navigate")
  assert.equal(opForTool("click"), "click")
...
test("tools/call maps a host error to an MCP isError result", async () => {
  ...
  const response = await server.handleRequest({
    ...
    id: 4,
    method: "tools/call",
    params: { name: "click", arguments: { ref: "x" } }
  })
