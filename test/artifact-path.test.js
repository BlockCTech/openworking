...
test("missing translated markdown artifacts return a neutral missing preview", () => {
  const project = fs.mkdtempSync(path.join(os.tmpdir(), "openworking-preview-md-missing-"))
  const artifact = path.join(project, "manual-translated-vietnamese.md")
  ...
  assert.throws(() => assertTranslationArtifact(project, artifact), /Artifact does not exist/)
})
...
test("translated artifact preview rejects invalid artifacts", () => {
  ...
  assert.throws(() => previewTranslationArtifact(project, path.join(project, "missing.md")), /not a translated document artifact/)
})
...
test("project directories list children inside the project tree", () => {
  ...
  // Relative paths cross the IPC boundary POSIX-style on every platform (see
  // normalizeProjectRelativePath), so this is "src/app.js" even on Windows — not path.join's
  // platform separator.
...
test("project directory listing can return only visible non-hidden non-gitignored files for @file", () => {
  ...
  assert.deepEqual(recursive.children, [
    { name: "app.js", path: "src/app.js", type: "file", openable: true },
    { name: "README.md", path: "README.md", type: "file", openable: true },
    { name: "report.md", path: "visible/nested/report.md", type: "file", openable: true },
    { name: "script.ts", path: "visible/nested/script.ts", type: "file", openable: true }
  ])
})
...
// PTY goal (M2): a renderer-supplied `cwd` for pty:create is gated through this same realpath
// boundary discipline before it's ever handed to the runtime — this is the one piece of that
// wiring testable outside Electron (main.js itself cannot be require()'d in a plain Node test:
// it calls Electron's `app.getPath` at module-eval time, which throws immediately outside a real
// Electron process — confirmed empirically; no ipcMain.handle body in this codebase has direct
// unit coverage for that reason, only the pure helpers it calls).
test("assertProjectDirectory accepts a directory inside the project and rejects one outside it", () => {
  const project = fs.mkdtempSync(path.join(os.tmpdir(), "openworking-ptycwd-project-"))
  const outside = fs.mkdtempSync(path.join(os.tmpdir(), "openworking-ptycwd-outside-"))
  const nested = path.join(project, "nested", "dir")
  fs.mkdirSync(nested, { recursive: true })
  const file = path.join(project, "not-a-dir.txt")
  fs.writeFileSync(file, "x")

  const result = assertProjectDirectory(project, "nested/dir")
  assert.equal(result.resolved, fs.realpathSync(nested))
  assert.equal(result.relative, path.join("nested", "dir"))

  // No cwd at all resolves to the project root itself (main.js's own fallback).
  const root = assertProjectDirectory(project, "")
  assert.equal(root.resolved, fs.realpathSync(project))

  assert.throws(() => assertProjectDirectory(project, outside), /outside the current project/)
  assert.throws(() => assertProjectDirectory(project, path.join(project, "does-not-exist")), /does not exist/)
  assert.throws(() => assertProjectDirectory(project, file), /not a directory/)
})
