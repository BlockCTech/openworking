...
- Clicking a project folder in the sidebar only expands or collapses its local session list; it never activates or restarts a runtime. Runtime activation remains in the existing startup flow and actions that open a session, start a new session, open a project card, or send a prompt.
...
- The model selector and its available variants are generated from OpenCode's runtime model catalog. The selected `{ providerID, id, variant? }` belongs to the session; changing it updates the session immediately without restarting the runtime.
...
- While a session is busy, **Stop** interrupts only the active execution, **Send/Enter** queues the next prompt, and the adjacent delivery menu exposes an explicit **Steer current run** action.
- Pending user inputs render inline with server-owned queued/running/applied state. Reconnect and reload hydrate the FIFO queue from OpenCode; transport-unknown retries reuse the same stable input ID. The pinned core has no durable per-item pending cancellation endpoint, so Stop does not remove queued inputs.
- The context popover can queue manual compaction while a session is idle or running, then shows queued/running/success/failure lifecycle state. After compaction, usage remains marked stale until the next model response reports real usage.
- Undo/Revert is limited to user-message boundaries. It stages conversation/file rollback behind a warning modal; a persistent banner exposes **Redo** and **Keep revert**, affected file paths, and the count of hidden messages. External attachments must be attached again manually.
