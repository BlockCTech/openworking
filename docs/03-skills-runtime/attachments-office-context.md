...
Với attachment tạm được sinh ra khi chuẩn hóa input, ownership đi theo `inputId`: file của input đang pending chỉ chuyển sang execution-owned khi nhận `session.input.promoted`. Idle/interrupt chỉ dọn file đã promote; queued input khác giữ file cho tới lượt chạy. Runtime shutdown dọn toàn bộ phần còn lại.
