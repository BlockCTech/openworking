...
Mục tiêu: agent trong OpenWorking có thể **điều khiển chính Chrome đã đăng nhập của người dùng**
...
Do **app tự định nghĩa** (ta tự viết MCP). MCP khai báo tên nội bộ `navigate`, `read`, `click`, `type`,
`screenshot`; OpenCode v2 thêm server prefix để expose trực tiếp thành `browser_navigate`,
`browser_read`, `browser_click`, `browser_type`, `browser_screenshot`. Không đặt sẵn prefix trong MCP,
nếu không provider wire sẽ nhận tên kép `browser_browser_*`. Giữ public surface này **đồng bộ** với
`askToolPermissions` của skill `browser-use`.
...
### Phân phối extension (bundled + popup hướng dẫn)
Extension ship kèm app dưới dạng unpacked trong `resources/browser-extension` — không tải từ đâu cả.
App hiển thị **popup HTML hướng dẫn cài** từng bước (mở thư mục bundled → `chrome://extensions` Load
unpacked → Install host) qua `renderBrowserSetupModal`, dùng IPC `browser:openExtensionPage` để mở thư
mục và `browser:installHost` để cài native host. Manifest **phải giữ nguyên `key`** để id vẫn là
...
- **Permission surface (extension)**: `manifest.json` xin `nativeMessaging`, `activeTab`, `scripting`,
  `tabs`, `alarms` và host access cho `http://*/*` + `https://*/*`. (`alarms` là heartbeat keep-alive cho
  service worker.) Native-host command không phải user gesture, nên `activeTab` một mình không cấp quyền
  cho `chrome.scripting.executeScript` hoặc `chrome.tabs.captureVisibleTab`; host access cho web pages là
  bắt buộc để browser tools chạy tự động sau khi điều hướng. Extension vẫn không truy cập được các trang
  đặc quyền như `chrome://extensions`. Reviewer coi app bridge (Task 5) là **security review**.
