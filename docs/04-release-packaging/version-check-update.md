...
> **Liên quan:** contract API phía server ở workspace-root [`../../../docs/upgrade_version.md`](../../../docs/upgrade_version.md) (không lặp lại ở đây).

## Context

Khi khởi động, desktop hỏi version-check API xem build hiện tại còn được hỗ trợ không, rồi (tùy cấu hình server) hiển thị **soft update** (gợi ý) hoặc **force update** (bắt buộc). Module này cũng tải và tự cài bản `.dmg` mới (macOS). `package.json` `version` là single source of truth (`app.getVersion()` → `current_version`).
...
- Gọi `GET <base>/api/v1/desktop-app/version?platform=…&current_version=…`. Base mặc định `VERSION_API_BASE = https://api-ai-console.beta.mynavitechtus.vn`, override env `OPENWORKING_VERSION_API_BASE`. Timeout `REQUEST_TIMEOUT_MS = 5s`.
...
- Contract API + soft/force semantics phía server: [`../../../docs/upgrade_version.md`](../../../docs/upgrade_version.md).
