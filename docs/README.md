...
| **Skills & Runtime** | `03-skills-runtime/` | [built-in-skills.md](03-skills-runtime/built-in-skills.md) | Bundle 15 skill offline, sync idempotent, slash commands, MCP/Extensions. |
...
| **Features** | `features/` | [worktree-branch-selector.md](features/worktree-branch-selector.md) | ★ Control chọn git worktree/branch cạnh "Execution" trong composer, ẩn khi project không phải git repo; chuyển worktree sẽ restart runtime nhưng không tạo entry project mới trong sidebar. |
| | | [vcs-changes-panel.md](features/vcs-changes-panel.md) | ★ As-built: tab `Changes` cạnh `Files` trong right sidebar — added/modified/deleted của worktree đang active qua `/api/vcs/*` (`vcs:*` IPC), lazy diff per-file mở bằng DocumentViewer; cạm bẫy `location[directory]` và lý do không dùng `filesystem.changed`. |
...
| | | [windows-build-release.md](04-release-packaging/windows-build-release.md) | ★ Windows x64/ARM64 NSIS packaging, native smoke tests, Azure Trusted Signing and update distribution. |
...
| | | [opencode-v2-feature-backlog.md](05-operations/opencode-v2-feature-backlog.md) | ★ Current delivery và TODO có priority/dependency/risk/acceptance signal cho các capability OpenCode Core v2 còn lại. |
